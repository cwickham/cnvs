...
Alternatively, from R, the default `cnvs()` endpoint is `/api/v1/courses` which will list all your courses:
...
Then you need to parse them and look for the `id` column in the appropriate row:

```{r}
#| label: courses-tibble
#| message: false
my_courses |>
  map(\(x) as_tibble(compact(x))) |>
  list_rbind()
```
